...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect.queen_counter == 0:
                return None
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        how_far = 0
        ThrowerAntPlace = self.place
        bees = self.place.bees
            #list of bees not in beehive
        while ThrowerAntPlace is not beehive:  #check the bees from current place
            if len(bees)>0:
                if how_far >= self.min_range and how_far <= self.max_range: # if there are bees then grab a random bees if not return none
                    return random_or_none(bees)
            ThrowerAntPlace = ThrowerAntPlace.entrance # go to the next place infront of it
            how_far += 1
            bees = ThrowerAntPlace.bees #get the bees from that place and go back to the while loop until the list is empty
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bees = list(self.place.bees)  #lists are pointers so we don't want to mutate all the bees so we create our a new list containing those bees in the same place as the ant.
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        elif self.digesting != 0: #why can't I put else? Isn't all the values except 0 a true value?
            self.digesting-=1
        # END Problem 6
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.queen_counter = QueenAnt.count
        QueenAnt.count +=1
        self.buffed_ants = []
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        queenPlace = self.place.exit
        if self.queen_counter == 0:
            ScubaThrower.action(self, colony)
            while queenPlace:
                if queenPlace.ant and queenPlace.ant not in self.buffed_ants:
                    queenPlace.ant.damage *= 2
                    self.buffed_ants += [queenPlace.ant]
                if queenPlace.ant and queenPlace.ant.is_container and queenPlace.ant.contained_ant and queenPlace.ant.contained_ant not in self.buffed_ants:
                    queenPlace.ant.contained_ant.damage *= 2
                    self.buffed_ants += [queenPlace.ant.contained_ant]
                queenPlace = queenPlace.exit
        else:
            Insect.reduce_armor(self, self.armor)

            #if current ants in buffed ants
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.queen_counter == 0:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    new_action, og_action = effect(bee.action, bee), bee.action
    counter = 0
