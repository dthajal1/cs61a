...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            symbol, value = formals, vals
            while symbol is not nil:
                local_frame.define(symbol.first, value.first)
                symbol = symbol.rest
                value = value.rest
            return local_frame
        raise SchemeError("Number of argument values doesn't match the number of formal parameters")
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Wrong numbers of arguments were passed')
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        local_frame = self.env.make_child_frame(self.formals, args)
        return local_frame
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        env.define(target, scheme_eval(expressions.rest.first, env)) #or env.define(expressions.first, target)
        return target
        # END PROBLEM 5
        ...
        target = LambdaProcedure(formals, body, env)  #have to bind the envioronemtn of lambda where you define it
        env.define(expressions.first.first, target)
...
def do_and_form(expressions, env):
    ...
    expr = expressions
    while expr:
        temp = scheme_eval(expr.first, env)
        if scheme_falsep(temp):
            return False
        expr = expr.rest
    return temp
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        value = Pair(scheme_eval(bindings.first.rest.first, env), value)
