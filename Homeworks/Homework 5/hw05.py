...
class VendingMachine:
    ...
    def __init__(self, candy_name, cost):
        self.candy_name = candy_name
        self.cost = cost
        self.balance = 0
        self.item_available = 0
    def vend(self):
        if self.item_available == 0:
            return 'Machine is out of stock.'
        elif self.balance == 0:
            return 'You must deposit ${0} more.'.format(self.cost)
        elif self.balance < self.cost:
            return 'You must deposit ${0} more.'.format(self.cost - self.balance)
        elif self.balance == self.cost:
            if True:
                b = 'Here is your {0}.'.format(self.candy_name)
            if True:
                self.balance = 0
                self.item_available -=1
                return b
        elif self.balance > self.cost:
            if True:
                a = 'Here is your candy and ${0} change.'.format(self.balance - self.cost)
            if True:
                self.balance = 0
                self.item_available -=1
                return a
    ...
    def restock(self, available):
        self.item_available += available
        return 'Current {0} stock: {1}'.format(self.candy_name, self.item_available)
...
def preorder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    ans = [t.label] #when we go call this functionn again on branch wouldn't ans be t.label again?
    if t.is_leaf():
        return ans
